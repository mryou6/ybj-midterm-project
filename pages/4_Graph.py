...
from components.graph_representation import (
    render_graph_representations,
)
...
render_concept_box(
    title="같은 그래프를 서로 다른 방식으로 저장할 수 있습니다.",
    text=(
        "인접 행렬은 정점 사이의 연결 여부를 0과 1로 나타내고, "
        "인접 리스트는 각 정점에 직접 연결된 정점만 목록으로 "
        "저장합니다. 위에서 만든 그래프를 두 표현 방식으로 "
        "비교해 보세요."
    ),
)
